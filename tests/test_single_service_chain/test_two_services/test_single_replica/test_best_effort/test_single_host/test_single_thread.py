...
sfc = pytest.sfc_two_services_one_thread


@pytest.mark.usefixtures("client", "server_app")
class Test1SFC2S1R1T1HBE:  # BE=Best Effort
    ...
    def test_all_traffic_types_all_topologies(self, traff_proto_name, topo_name, expected_avg_lats_obj, ras, filepath):
        if traff_proto_name == "1batchps_60sec_2paralleluser" and \
                topo_name == "tau_1host_2cores_host_scenario":
            m: SimulationScenarioManager = pytest.exec_scenario(
                ...
                sfc=sfc,
                microservices=[pytest.ms1_single_thread, pytest.ms2_single_thread],
                replica_count=1,
